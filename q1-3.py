...
def calculate_collision(hash_table):
    # function to calculate the number of collisions in a hash table
    # a collisions is counted when more than one item hashes to the same index

    # initialize collision counter to 0
    collision = 0

    # loop through each chain in the hash table
    for chain in hash_table.values():
        # if a chain has more than one item, a collision occurs.
        # subtract 1 to count how many extra items were inserted, that is the collisions.
        collision += len(chain)-1
...
def main():
    ...
    # list to store the number of collisions in each round
    collisions_per_round_table_1 = []
    collisions_per_round_table_2 = []

    # initialize variables to store total collisions for computing averages
    total_collisions_table_1 = 0
    total_collisions_table_2 = 0
    ...
    for round in range(1, num_of_rounds + 1):
        ...
        # initialize empty hash tables for each round
        hash_table_1 = {}
        hash_table_2 = {}
        ...
        # calculate and display the maximum chain length for each table
        ...
        # calculate the number of collisions for each table in each round
        collisions_1 = calculate_collision(hash_table_1)
        collisions_2 = calculate_collision(hash_table_2)

        # store the collisions into both lists for analysis
        collisions_per_round_table_1.append(collisions_1)
        collisions_per_round_table_2.append(collisions_2)

        # sum up the total collisions to compute average collision
        total_collisions_table_1 += collisions_1
        total_collisions_table_2 += collisions_2

    # calculate the average collisions
    average_collision_1 = total_collisions_table_1 / num_of_rounds
    average_collision_2 = total_collisions_table_2 / num_of_rounds

    # display final summary results
    print(f"----------------------Summary----------------------")
    print(f"Collisions per Round for Table 1: {collisions_per_round_table_1}")
    print(f"Collisions per Round for Table 2: {collisions_per_round_table_2}\n")
    print(f"Average Collisions for Table 1: {average_collision_1}")
    print(f"Average Collisions for Table 2: {average_collision_2}")
    print("---END---")
